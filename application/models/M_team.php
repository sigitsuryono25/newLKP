...
class M_team extends CI_Model {
    ...
    function daftarTeamBy($idTeam) {
        return $this->db->get_where('team', array('id' => $idTeam))->result();
    }

    function daftarTeam() {
        return $this->db->query('SELECT * FROM team INNER JOIN posisi ON team.posisi=posisi.id_posisi ORDER BY id')->result();
    }
